...
const datasetId = "adops_dashboard"; // PRODUCTION dataset
...
async function diagnoseProductionData() {
  ...
  try {
    ...
    // 3. Row count by sync_id (to see sync history)
    ...
    syncStates.forEach(row => {
      const syncedAt = row.synced_at?.value || row.synced_at;
      console.log(`${row.sync_id.padEnd(25)} | ${syncedAt} | ${row.status.padEnd(10)} | ${row.mode || 'N/A'} | Rows: ${row.row_count || 0}`);
    });

    // 5. Check for missing countries
    const configuredCountries = [
      "USA", "UK", "India", "Canada", "Middle East", "Malaysia", "Singapore",
      "Australia", "Indonesia", "Japan", "New Zealand", "Pakistan", "Philippines",
      "South Africa", "Thailand", "Vietnam"
    ];

    const countriesInData = new Set(countryRows.map(r => r.country));
    const missingCountries = configuredCountries.filter(c => !countriesInData.has(c));

    if (missingCountries.length > 0) {
      console.log("\n⚠️  Missing Countries (configured but no data):");
      console.log("─".repeat(80));
    ...
    const [dataQuality] = await bigquery.query({
      query: `
        SELECT 
          COUNT(*) as total_rows,
          COUNT(DISTINCT campaign_id) as unique_campaigns,
          SUM(CASE WHEN revenue = 0 AND spend = 0 THEN 1 ELSE 0 END) as zero_value_rows,
    ...
    console.log(`Unique Campaigns: ${quality.unique_campaigns}`);
    console.log(`Zero Value Rows: ${quality.zero_value_rows}`);
    console.log(`Missing Month: ${quality.missing_month}`);
    console.log(`Missing Year: ${quality.missing_year}`);
